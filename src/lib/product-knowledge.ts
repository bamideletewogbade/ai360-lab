...
export function productKnowledgeBlock() {
  ...
  return [
    ...
    `- Everyday chat on the fast model is included with a plan up to a fair daily limit (Explorer 10, Everyday 60, Builder 120, Team 150 messages, resetting at midnight UTC); extra chat messages cost 1 credit each. Live web research, attached files, deliberately premium models (Claude Sonnet 5, Kimi K3), multi-step agent workflows, images and video draw from a credit balance. Work holds credits only while it runs; failed work returns them; no task costs more than the amount shown before it starts.`,
    `- Plans: ${plans}.`,
    `- Payment is by Mobile Money (MTN MoMo, Telecel Cash, AT Money) or card, one month at a time during the pilot. There are no annual plans yet, and nothing renews without the person starting a fresh payment. One-time top-ups of 40, 90 or 185 credits are available from the credit page; they cost more per credit than a plan and never renew.`,
    `- What it can do: everyday chat, live web search with cited sources, reading attached files and PDFs, multi-step research agents, Studio packs (brand and launch, marketing, ads, name and domain, pitch, content calendar), image generation, short promotional video with a quote shown before it runs, document export, and replies in English, ${languages}.`,
