...
export type MediaJobStatus = 'queued' | 'submitted' | 'running' | 'completed' | 'failed' | 'cancelled'
...
export async function createMediaJob(input: {
  ...
  return sql.begin(async (tx) => {
    ...
    const rows = await tx.unsafe<MediaJobRow[]>(`${JOB_SELECT}
...
export async function readMediaJob(context: WorkspaceAuthContext, jobId: string) {
  const sql = getPostgres()
  const rows = await sql.unsafe<MediaJobRow[]>(`${JOB_SELECT}
    where job.workspace_key = $1 and job.id = $2 limit 1`,
    [context.workspace.key, jobId.slice(0, 96)])
  return rows[0] ? mapJob(rows[0]) : null
}

export async function listMediaJobs(context: WorkspaceAuthContext, projectId: string, limit = 50) {
  const sql = getPostgres()
  const rows = await sql.unsafe<MediaJobRow[]>(`${JOB_SELECT}
    where job.workspace_key = $1 and job.project_id = $2
    order by job.created_at desc limit $3`,
    [context.workspace.key, projectId.slice(0, 64), Math.min(100, Math.max(1, limit))])
