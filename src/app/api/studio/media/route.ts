...
import { isMediaJobStoreConfigured, listMediaJobs, readMediaJob } from '@/lib/media/job-repository'
...
export async function GET(request: Request) {
  ...
  const limited = rateLimit(request, 'studio_video_status', { minute: 40, daily: 600 })
  ...
  try {
    ...
    if (!context) {
      ...
      return Response.json({ error: 'Sign in to open saved media.' }, { status: 401, headers: log.headers() })
    }
    if (!isMediaJobStoreConfigured()) {
      log.finish(503, { outcome: 'not_configured' })
      return Response.json({ error: 'Saved media is not configured.' }, { status: 503, headers: log.headers() })
    }
    ...
    if (assetId) {
      ...
      if (!asset) {
        ...
        return Response.json({ error: 'This media file was not found.' }, { status: 404, headers: log.headers() })
      }
    ...
    if (jobId) {
      ...
      return Response.json(job ? { job } : { error: 'This media job was not found.' }, {
        status: job ? 200 : 404,
    ...
    if (!projectId) {
      ...
      return Response.json({ error: 'Choose a project or media job.' }, { status: 400, headers: log.headers() })
    }
    const jobs = await listMediaJobs(context, projectId)
    log.finish(200, { outcome: 'list', count: jobs.length })
    return Response.json({ jobs }, { headers: log.headers({ 'Cache-Control': 'no-store' }) })
  } catch (error) {
    log.error('studio.media.failed', errorDetails(error))
    log.finish(500, { outcome: 'exception' })
    return Response.json({ error: 'Saved media could not be opened.' }, { status: 500, headers: log.headers() })
  }
}
