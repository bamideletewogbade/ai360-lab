...
we do now and what leading products do, resolved for _our_ constraints, not a
copy of theirs.
...
- **Stack**: Next.js (App Router), Supabase Auth (identity â€” the only auth authority; Clerk was retired 2026-08-14), Supabase Postgres (the only data plane, RLS on every table), OpenRouter (AI gateway), ExpressPay (hosted MoMo checkout, live for the prepaid pilot). Guest-first: people can use the product before signing in, and their local state should follow them into their account on sign-in (claim-on-login).
...
(engineering blogs, docs, credible write-ups). Capture the _pattern and the
why_, not surface UI. Cite sources as markdown links. If a search is thin,
...
and the Supabase Auth + Postgres stack. Structure it as:
